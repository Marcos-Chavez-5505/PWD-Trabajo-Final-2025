...
class UsuarioRol {
    ...
    public function __construct($objPdo = null) {
        ...
        $this->objRol = null;
        $this->objPdo = $objPdo ?? new bdCarritoCompras();
    }
    ...
    public function modificar(){
        ...
        if ($this->objPdo->Iniciar()){
            $idusuario = $this->idusuario ?? $this->getObjUsuario()->getIdusuario();
            $idrol = $this->idrol ?? $this->getObjRol()->getIdrol();
            $sql = "UPDATE usuariorol SET idusuario = $idusuario, idrol = $idrol
    ...
    public function eliminar(){
        ...
        if ($this->objPdo->Iniciar()){
            $idusuario = $this->idusuario ?? $this->getObjUsuario()?->getIdusuario();
            $idrol = $this->idrol ?? $this->getObjRol()?->getIdrol();
            $sql = "DELETE FROM usuariorol WHERE idusuario = " . intval($idusuario) .
                   " AND idrol = " . intval($idrol);
            if ($this->objPdo->Ejecutar($sql) >= 0) $rta = true;
        }
        return $rta;
    }
